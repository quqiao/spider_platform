...
def crawl_longyi_tjzq_pt():
    ...
    for i in range(1, 32):
        ...
        for j in range(1, 21):
            jg = html.xpath('/html/body/div[8]/div[4]/div/ul/li[%d]/div[2]/span[1]/text()' % j)
            jg1 = ''.join(jg)
            list_jiage.append(jg1)
        for j in range(1, 21):
            jg2 = html.xpath('/html/body/div[8]/div[4]/div/ul/li[%d]/div[2]/span[2]/text()' % j)
            jg3 = ''.join(jg2)
        ...
        for x in range(1, 21):
            xq = html.xpath('/html/body/div[8]/div[4]/div/ul/li[%d]/p[3]/span[1]/text()' % x)
            xq1 = ''.join(xq)
...
"""保存为csv格式文件"""
def save_csv():
    dataframe = pd.DataFrame({'特价价': list_jiage, '原价': list_jiage2, '药名': list_mingzi, '厂家': list_compamy, '规格': list_guige, '效期': list_xiaoqi, '限购': list_xiangou})  # 字典中的key值即为csv中列名
    dataframe.to_csv("scjuchuang_bptj_20201102_xiangou.csv", index=False, sep=',')  # 将DataFrame存储为csv,index表示是否显示行名，default=True
