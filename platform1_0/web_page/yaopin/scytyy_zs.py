...
def crawl_scytyy_yp():
    ...
    time.sleep(1)
    for i in range(1, 10):
        driver.get("http://www.scjzyt.com/goods-zs-filter-0-0-0-0-0-0-0-1-1-%d,.html" % i)
        time.sleep(3)  # 停顿3秒等待页面加载完毕！！！（必须留有页面加载的时间，否则获得的源代码会不完整。）
        html_sourcode = driver.page_source
        html = etree.HTML(html_sourcode, etree.HTMLParser())
        for j in range(1, 21):
            jg = html.xpath('//*[@id="pro_list1"]/li[%d]/p[1]/span[2]/text()' % j)
            jg1 = ''.join(jg)
            list_jiage1.append(jg1)

            jg3 = html.xpath('//*[@id="pro_list1"]/li[%d]/p[1]/span[1]/text()' % j)
            jg4 = ''.join(jg3)
            list_jiage2.append(jg4)

            jg5 = html.xpath('//*[@id="pro_list1"]/li[%d]/p[1]/span[1]/span/text()' % j)
            jg6 = ''.join(jg5)
            list_jiage3.append(jg6)

            # jg7 = html.xpath('//*[@id="pro_list1"]/li[%d]/p[1]/span/text()' % j)
            # jg8 = ''.join(jg7)
            # list_jiage4.append(jg8)
...
def save_csv():
    """使用csv保存数据"""
    # dataframe = pd.DataFrame({'原价': list_jiage1, '活动价':list_jiage2, '银卡价': list_jiage3,  '厂家特供价': list_jiage4,
    #                 '药名': list_mingzi, '厂家': list_compamy, '规格': list_guige, '效期': list_xiaoqi})  # 字典中的key值即为csv中列名
    # dataframe.to_csv("hezongyy_20201111.csv", index=False, sep=',')  # 将DataFrame存储为csv,index表示是否显示行名，default=True

    """通过xlsx保存数据"""
    data = {'原价': list_jiage1, '活动价':list_jiage2, '银卡价': list_jiage3,
                    '药名': list_mingzi, '厂家': list_compamy, '规格': list_guige, '效期': list_xiaoqi}
    dataframe = pd.DataFrame(data, columns=['原价', '活动价', '银卡价',  '药名', '厂家', '规格', '效期'])
    dataframe.to_excel("scyt_zs_20210324.xlsx", encoding='utf-8', index=False, header=True, sheet_name='粤通品种')
    print(dataframe)

    """在已有的excel中加入新的sheet保存数据"""
    # nowtime = time.strftime('%Y%m%d', time.localtime(time.time()))  # 获取当前时间并转化为类似20201217的格式
    # try:
    #     data = {'原价': list_jiage1, '活动价':list_jiage2, '银卡价': list_jiage3,  '厂家特供价': list_jiage4,
    #             '药名': list_mingzi, '厂家': list_compamy, '规格': list_guige, '效期': list_xiaoqi}
    ...
    #     df = pd.DataFrame(data, columns=['原价', '活动价', '银卡价', '厂家特供价', '药名', '厂家', '规格',
    ...
    #     data = {'原价': list_jiage1, '活动价':list_jiage2, '银卡价': list_jiage3,  '厂家特供价': list_jiage4,
    #             '药名': list_mingzi, '厂家': list_compamy, '规格': list_guige, '效期': list_xiaoqi}
    #     dataframe = pd.DataFrame(data, columns=['原价', '活动价', '银卡价', '厂家特供价', '药名', '厂家', '规格', '效期'])
