...
def crawl_longyi_tjzq_pt():
    ...
    username.send_keys('13808274442')
    ...
    for i in range(1, 79):
        driver.get("https://www.scjuchuang.com/goods?id=&name=&pid=&pname=&valid_time=&tname=&agent_id=&aname=&activity_type=0&acname=&sort_order=0&sort_type=1&is_stock=0&search=%s&approval_number=&attr=&page=%d" % ("注射",i))
        time.sleep(3)  # 停顿3秒等待页面加载完毕！！！（必须留有页面加载的时间，否则获得的源代码会不完整。）
        html_sourcode = driver.page_source
        html = etree.HTML(html_sourcode, etree.HTMLParser())
        for j in range(1, 21):
            jg = html.xpath('/html/body/div[8]/ul/li[%d]/p[3]/span[2]/text()' % j)
            jg1 = ''.join(jg)
            list_jiage.append(jg1)
        for j in range(1, 21):
            jg2 = html.xpath('/html/body/div[8]/ul/li[%d]/div[1]/span/text()' % j)
            jg3 = ''.join(jg2)
        ...
        for x in range(1, 21):
            xq = html.xpath('/html/body/div[8]/ul/li[%d]/p[4]/span[1]/text()' % x)
            xq1 = ''.join(xq)
...
def save_csv():
    ...
    dataframe.to_csv("scjuchuang_zj_20210311.csv", index=False, sep=',')  # 将DataFrame存储为csv,index表示是否显示行名，default=True
