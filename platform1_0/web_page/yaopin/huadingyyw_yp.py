...
def crawl_huadingyyw_yp():
    ...
    driver.find_element_by_class_name('is').click()  # 模拟点击“登录”按钮
    time.sleep(1)
    driver.maximize_window()  # 切换为全屏显示
    ...
    for i in range(1, 2):
        ...
        for i in range(1, 21):
            jg = html.xpath('//*[@id="pro_list1"]/li[%d]/p[1]/span[1]/text()' % i)
            jg1 = ''.join(jg)
            list_jiage.append(jg1)

            cj = html.xpath('//*[@id="pro_list1"]/li[%d]/p[3]/text()' % i)
            cj1 = ''.join(cj)
            list_compamy.append(cj1)

            mz = html.xpath('//*[@id="pro_list1"]/li[%d]/p[2]/a/text()' % i)
            mz1 = ''.join(mz)
            list_mingzi.append(mz1)

            gg = html.xpath('//*[@id="pro_list1"]/li[%d]/p[4]/text()' % i)
            gg1 = ''.join(gg)
            list_guige.append(gg1)

            xq = html.xpath('//*[@id="pro_list1"]/li[%d]/div[2]/div[1]/span/text()' % i)
            xq1 = ''.join(xq)
...
def save_csv():
    ...
    # dataframe.to_excel("hezongyy_20201111.xlsx", encoding='utf-8', index=False, header=True, sheet_name='华鼎')
    # print(dataframe)
    ...
    try:
        ...
        df = pd.DataFrame(data, columns=['原价', '药名', '厂家', '规格', '效期'])  # 如果有相同名字的工作表，新添加的将命名为Sheet21，如果Sheet21也有了就命名为Sheet22，不会覆盖原来的工作表
        ...
        dataframe.to_excel('F:/django/spider_platform/DataAnalysis/data/medical_data_%s.xlsx' % nowtime, encoding='utf-8', index=False, header=True, sheet_name='华鼎')
