...
def crawl_longyi_tjzq_pt():
    ...
    for i in range(1, 41):
        driver.get("https://www.scjuchuang.com/newTemplate?theme=3&group_id=4210&isset_cart=&id=131&order_type=&order=&activity_type=1&page=%d" % i)
...
def save_csv():
    ...
    dataframe.to_csv("scjuchuang_tjzq_20210311.csv", index=False, sep=',')  # 将DataFrame存储为csv,index表示是否显示行名，default=True
